...
class OtherData extends LivewireDatatable
{
    ...
    public function builder()
    {
        ...
        if (Session::get('savingsViewItem') == '3') {
            return AccountsModel::query()->where('account_status', 'Active')->where('product_number', '13');
        }
        if (Session::get('savingsViewItem') == '7') {
            return AccountsModel::query()->where('account_status', 'Inactive')->where('product_number', '13');
        }
        if (Session::get('savingsViewItem') == '4') {
            return AccountsModel::query()->where('account_status', 'Blocked')->where('product_number', '13');
        }
        if (Session::get('savingsViewItem') == '5') {
            return AccountsModel::query()->where('account_status', 'Pending')->where('product_number', '13');
        }
        if (Session::get('savingsViewItem') == '6') {
            return AccountsModel::query()->where('account_status', 'Closed')->where('product_number', '13');
        }
