...
class Accounting extends Component
{
    ...
    public function updatedAccountsAccount(){
        $AccountsAccountData = AccountsModel::select('membershipNumber', 'name', 'region', 'wilaya', 'email')
        ->where('id', '=', $this->AccountsAccount)
        ->get();
    foreach ($AccountsAccountData as $AccountsAccount){
        $this->membershipNumber=$AccountsAccount->membershipNumber;
        $this->name=$AccountsAccount->name;
        $this->region=$AccountsAccount->region;
        $this->wilaya=$AccountsAccount->wilaya;
        $this->email=$AccountsAccount->email;
        $this->status=$AccountsAccount->status;
    }
    }
    ...
    public function updateAccountsAccount(){
        ...
        $update_value = approvals::updateOrCreate(
            ...
            [
                'institution' => $this->AccountsAccount,
                'process_name' => 'editAccountsAccount',
                'process_description' => 'has edited a AccountsAccount',
                'approval_process_description' => 'has approved changes to a AccountsAccount',
                'process_code' => '02',
                'process_id' => $this->AccountsAccount,
    ...
    public function editAccountsAccountModal($id)
    {
        ...
        $this->pendingAccountsAccountname = AccountsModel::where('id',$id)->value('name');
