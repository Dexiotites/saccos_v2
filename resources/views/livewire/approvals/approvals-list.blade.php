                    ...
                    <div class="flex-1 pl-2">
                        ...
                        @foreach(App\Models\Branches::where('id',$approval->process_id)->get() as $branch)
                            <div class="flex-1">
                                ...
                                <p class="text-gray-600 font-thin">
                                    {{$branch->region}} - {{$branch->wilaya}} : Membership No {{$branch->membershipNumber}}
                                </p>
                            </div>
                        ...
                        @if( $approval->process_code == '09')
                            @foreach(App\Models\AccountsModel::where('id',$approval->process_id)->get() as $accounts)
                                <div class="flex-1">
                                    <p class="text-gray-700 font-semibold">
                                        Member Name : {{App\Models\Members::where('membership_number',$accounts->member_number)->value('first_name')}}
                                        {{App\Models\Members::where('membership_number',$accounts->member_number)->value('middle_name')}}
                                        {{App\Models\Members::where('membership_number',$accounts->member_number)->value('last_name')}}
                                        {{App\Models\Members::where('membership_number',$accounts->member_number)->value('business_name')}}
                                    </p>
                                    <p class="text-gray-700 font-semibold">
                                        Membership Type : {{App\Models\Members::where('membership_number',$accounts->member_number)->value('membership_type')}}
                                    </p>
                                    <p class="text-gray-700 font-semibold">
                                        Membership Number : {{$accounts->member_number}}
                                    </p>
                                    <p class="text-gray-700 font-semibold">
                                        Branch : {{App\Models\Branches::where(
                                                'id',
                                                App\Models\Members::where('membership_number',$accounts->member_number)->value('branch')
                                                )->value('name')}}
                                    </p>
                                    <p class="text-gray-600 font-thin">
                        ...
                        @if( $approval->process_code == '12')
                            @foreach(App\Models\Members::where('id',$approval->process_id)->get() as $member)
                                ...
                                <div class="w-full md:flex mt-2 flex justify-end" >
                                    ...
                                    <div class="flex mr-4" >
                                        <div wire:loading wire:target="rejectCreateMember({{$member->id}},'{{$approval->id}}')">
                                            <button class="text-white bg-blue-400 hover:bg-blue-400 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-2 dark:bg-blue-400 dark:hover:bg-blue-400 dark:focus:ring-blue-400"
                                                    disabled>
                                                <div class="flex items-center">
                                                    <svg xmlns="http://www.w3.org/2000/svg"
                                        ...
                                        <div wire:loading.remove wire:target="rejectCreateMember({{$member->id}},'{{$approval->id}}')">

                                            @php

                                            @endphp
                                            <button wire:click="rejectCreateMember({{$member->id}},'{{$approval->id}}')"
                                                    class="text-white bg-red-400 hover:bg-red-400 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm px-4 py-2 dark:bg-red-400 dark:hover:bg-red-400 dark:focus:ring-red-400">
                                                <p class="text-white">Reject</p>
                                            </button>
                                    ...
                                    <div class="flex">
                                        <div wire:loading wire:target="approveCreateMember({{$member->id}},'{{$approval->id}}')">
                                        ...
                                        <div wire:loading.remove wire:target="approveCreateMember({{$member->id}},'{{$approval->id}}')">
                                            <button @if(
                                            ...
                                            wire:click="approveCreateMember({{$member->id}},'{{$approval->id}}')"
                                                    class="text-white bg-blue-400 hover:bg-blue-400 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-2 dark:bg-blue-400 dark:hover:bg-blue-400 dark:focus:ring-blue-400">
                                                <p class="text-white">Approve</p>
                                            </button>
                        ...
                        @if( $approval->process_code == '14')
                            @foreach(App\Models\institutions::where('id',$approval->process_id)->get() as $institution)
                                <div class="flex-1 mb-4">
                                    ...
                                    <p class="text-gray-600 font-thin">
                                        {{$institution->region}} - {{$institution->wilaya}} : Membership No {{$institution->membershipNumber}}
                                    </p>
                                    @php
